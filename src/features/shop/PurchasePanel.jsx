...
export function PurchasePanel({ selectedProduct, quantity, loading, isSignedIn, onQuantityChange, onCreateOrder, onAddToCart }) {
  ...
  return (
    <section className="purchase-panel">
      ...
      <div className="purchase-actions">
        <button className="secondary-button cart-add-button" type="button" disabled={loading || isLimitedProduct} onClick={onAddToCart}>
          <ShoppingCart size={18} />
          {isLimitedProduct ? '한정 상품은 바로 주문만 가능' : '장바구니에 담기'}
        </button>
        <button className="primary-button purchase-button" type="button" disabled={!isSignedIn || loading} onClick={onCreateOrder}>
          <ShoppingBag size={18} />
